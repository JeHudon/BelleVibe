...
function BadgeStatut({ statut }) {
	...
	if (statut === "en attente") return <span className="tag">En attente</span>;
			...
			<div className="box" style={{ border: "1px solid #e8e8e8" }}>
				...
				>
					...
					<tbody>
						{chargement ? (
							<tr>
								<td colSpan="7" className="has-text-centered py-4">
									Chargement...
								</td>
							</tr>
							...
							dossiersFiltres.map((d) => (
								<tr key={d.idDossier} style={{ height: "55px" }}>
									...
									<td className="is-vcentered">
										<span
											...
											onClick={() => navigate(`/clients/${d.idClient}/resume`)}
										>
									...
									<td className="is-vcentered">
										<span
											className={
												d.soldeDossier < 0 ? "has-text-danger" : "has-text-success"
											}
										>
