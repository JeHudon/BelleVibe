...
export default function GestionClients() {
  ...
  async function sauvegarderModification(e) {
    ...
    try {
      const res = await fetch(
        `/api/clients/modifierClient/${clientAModifier.idClient}`,
        {
          method: "PUT",
          headers: {
  ...
  async function supprimerClient() {
    try {
      const res = await fetch(
        `/api/clients/supprimerClient/${clientASupprimer.idClient}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        },
      ...
      setClients((prev) =>
        prev.filter((c) => c.idClient !== clientASupprimer.idClient),
      );
        ...
        >
          <thead>
            <tr>
              ...
              <th>Adresse</th>
              <th style={{ width: "90px" }}>Comptes</th>
          ...
          <tbody>
            {chargement ? (
              ...
              clientsFiltres.map((c) => {
                ...
                return (
                  <tr key={c.idClient} style={{ height: "55px" }}>
